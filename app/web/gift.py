from app.libs.enums import PendingStatus
from app.models.drift import Drift
from app.models.gift import Gift
from app.view_models.gift import MyGiftViewModels
from . import web
from flask import render_template, current_app,flash,redirect,url_for
from app.models.base import db
from flask_login import login_required,current_user
__author__ = '七月'


@web.route('/my/gifts')
@login_required
def my_gifts():
    gifts_of_mine = Gift.get_user_gifts(current_user.id)
    isbn_list = [gift.isbn for gift in gifts_of_mine]
    wishes_count_list = Gift.get_wish_counts(isbn_list)
    giftViewModel = MyGiftViewModels(gifts_of_mine,wishes_count_list)
    print("*************************")
    print(giftViewModel.gifts)
    return render_template("my_gifts.html", gifts=giftViewModel.gifts)



@web.route('/gifts/book/<isbn>') #赠送此书
@login_required
def save_to_gifts(isbn):
    if current_user.can_save_to_list(isbn):
        # try: #使用Contextlib制作的上下文管理器，进行自动commit
        with db.auto_commit():
            gift = Gift()
            gift.isbn = isbn
            gift.uid = current_user.id
            current_user.beans += current_app.config["BEANS_UPLOAD_ONE_BOOK"]
            db.session.add(gift)

            # db.session.commit()
        # except Exception as e:
        #     db.session.rollback()
        #     raise e
    else:
        flash("这本书已添加到你的赠送清单或已存在你的心愿清单，请不要重复添加")

    return redirect(url_for("web.book_detail", isbn=isbn))


@web.route('/gifts/<gid>/redraw')
def redraw_from_gifts(gid):
    """撤销礼物，先查询在drift中是否在witing状态"""
    gift = Gift.query.filter_by(id=gid, uid=current_user.id, launched=False).first_or_404()
    drift = Drift.query.filter_by(gift_id=gid,gifter_id=current_user.id, pending=PendingStatus.Witing).first()
    if drift:
        flash("这个礼物正处于交易状态，请先到鱼漂中处理此书")
    else:
        with db.auto_commit():
            current_user.beans -= current_app.config["BEANS_UPLOAD_ONE_BOOK"]
            gift.delete()

    return redirect(url_for("web.my_gifts"))


